document.addEventListener('DOMContentLoaded', () => {
    ...
    if (flippedCards.length === 2) {
      ...
      if (c1.dataset.emoji === c2.dataset.emoji) {
        ...
        lockBoard = false;
          ...
          mascotSay(pickRandom(phrases.memory));
  ...
  const sprites = {
    idle: 'assets/animations/Purple_Cute_Cat_idle_64x64.png',
    happy: 'assets/animations/Purple_Cute_Cat_happy_64x64.png',
    celebrate: 'assets/animations/Purple_Cute_Cat_celebrate_64x64.png',
    sleep: 'assets/animations/Purple_Cute_Cat_sleep_64x64.png',
    eat: 'assets/animations/Purple_Cute_Cat_eat_64x64.png',
    run: 'assets/animations/Purple_Cute_Cat_run_64x64.png',
    walk: 'assets/animations/Purple_Cute_Cat_walk_64x64.png',
  };

  const phrases = {
    happy: ['¡Bien hecho! 💜', '¡Qué bien! ⭐', '¡Super! ✨', '¡Me encanta! 🎉', '¡Genial! 🥳'],
    celebrate: ['¡Fiesta! 🎊', '¡Bravísimo! 🏆', '¡Espectacular! 🌟', '¡Lo lograste! 🎈', '¡WOW! 🎯'],
    greet: ['¡Hola! 💜', 'Teacher Sara 🥰', '¡A aprender! 📚', 'Hello! 👋', '¿Jugamos? 🎮'],
    points: ['¡Sigue así! ⭐', 'Estrella para ti 🌟', '¡Punto ganado! 💪', 'Excelente trabajo 👏'],
    wheel: ['¡A girar! 🎡', '¿Qué saldrá? 🤔', '¡Otra vez! 🌀', '¡Suerte! 🍀'],
    memory: ['¡Pareja encontrada! 🧠', 'Buena memoria 🐱', '¡Dos iguales! 💜', '¡Eres un crack! 🏅'],
    flashcard: ['¡Nueva palabra! 📖', 'Aprendiendo inglés 🎓', '¡Qué palabra! ✏️', 'Estudia estudia 📝'],
    idle: ['💜', '😊', '🐱', '✨', '🎵'],
    sleep: ['Zzz 💤', 'Descansando... 😴', 'Hora de la siesta 🛏️', 'Zzzz... 💜'],
  };
  ...
  function setMascotImage(spriteKey) {
    if (sprites[spriteKey]) {
      mascotImg.src = sprites[spriteKey];
    }
    mascot.classList.remove('idle', 'happy', 'celebrate', 'sleep');
    mascotState = spriteKey;
    if (spriteKey !== 'sleep') {
      mascot.classList.remove('sleep');
      mascotTooltip.textContent = '💜';
    }
  }
  ...
  function showBubble(text, duration = 2500) {
    ...
    mascotBubble._hideTimer = setTimeout(() => {
  ...
  function mascotHappy() {
    ...
    showBubble(pickRandom(phrases.happy));
    resetInactivityTimer();
    setTimeout(() => {
      if (mascotState === 'happy') setMascotImage('idle');
    }, 2500);
  }

  function mascotCelebrate() {
    setMascotImage('celebrate');
    mascot.classList.add('celebrate');
    showBubble(pickRandom(phrases.celebrate));
    resetInactivityTimer();
    setTimeout(() => {
      if (mascotState === 'celebrate') setMascotImage('idle');
    }, 2500);
  }

  function mascotSay(message, duration = 2500) {
  ...
  function resetInactivityTimer() {
    ...
    inactivityTimer = setTimeout(() => {
      setMascotImage('sleep');
      mascot.classList.add('sleep');
      mascotTooltip.textContent = '💤';
      showBubble(pickRandom(phrases.sleep), 2000);
    }, 30000);
  }

  // Click en la mascota: reacción
  mascot.addEventListener('click', () => {
    if (mascotState === 'sleep') {
      setMascotImage('happy');
      mascot.classList.add('happy');
      showBubble('¡Estoy despierto! 💜');
      resetInactivityTimer();
      setTimeout(() => setMascotImage('idle'), 2000);
    } else {
      mascotHappy();
    }
  });
  ...
  pointsAdd.addEventListener('click', () => {
    if (points % 5 === 0 && points > 0) {
      ...
      mascotSay(pickRandom(phrases.points));
    }
  });

  // Ruleta
  spinBtn.addEventListener('click', () => {
    setTimeout(() => mascotSay(pickRandom(phrases.wheel)), 3500);
  });

  // Memory - ya está integrado arriba en flipMemoryCard

  // Flashcard
  flashcardBtn.addEventListener('click', () => {
    mascotSay(pickRandom(phrases.flashcard));
  });
  ...
  tabBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      mascotSay('💜 ' + btn.textContent.trim(), 1800);
    });
  });
  ...
  setTimeout(() => {
    ...
    showBubble(greeting + ' ' + pickRandom(phrases.greet), 3000);
    resetInactivityTimer();
  }, 1500);
